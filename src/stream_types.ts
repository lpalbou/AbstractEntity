...
export interface DisplayBlock {
  ...
  entry_id?: string;
  /** Diary projection type (question | problem | idea | note | …) — live
   * on diary bindings; drives the hint chip's kind tint. */
  diary_type?: string;
...
export interface TracePayload {
  ...
  channels: string[];
  /** DISPLAY-BOUNDED list (the engine's trace cap — 100 since the operator's
   * 2026-08-01 "at most a 100" alignment; 64 before it). Never count this
   * for "N considered": prefer budget_spent.candidates_considered (the
   * engine's own accounting) via stream_fold.consideredCount. */
  ...
  /** Engine accounting for the recall (rides the trace whole; see
   * abstractmemory reconstruct.py budget_spent): candidates_considered is
   * the TRUE pool count — the authoritative "N considered". Optional:
   * older persisted traces may predate it. */
  budget_spent?: Record<string, unknown>;
  seq: number;
}
