...
function storageKey(entity: string): string {
  return `abstractentity_substrate:${entity}`;
}

/** The stored per-entity choice; null when the operator has not chosen.
 * Reads the pre-split observer spelling as a one-time fallback (repo split
 * 2026-07-12, same origin as the old entity launcher) — a durable per-entity
 * operator pick should survive a rename like layout anchors do. */
export function loadSubstrateChoice(entity: string): SubstrateChoice | null {
  try {
    const raw =
      localStorage.getItem(storageKey(entity)) ?? localStorage.getItem(`abstractobserver_entity_substrate:${entity}`);
    if (!raw) return null;
