...
describe("tokens never rest client-side (uic c1142 ruling, not dev-waivable)", () => {
  it("storeAuth persists NOTHING — even a remembered bearer", () => {
  ...
  it("loadStoredAuth SCRUBS tokens persisted by earlier builds (migration, incl. pre-split observer spellings)", () => {
    const store = stubStorage();
    store.set(AUTH_STORAGE_KEY, JSON.stringify({ mode: "bearer", userId: "op", token: "old-tok", remembered: true }));
    store.set("abstractobserver_gateway_auth", JSON.stringify({ mode: "bearer", userId: "op", token: "pre-split", remembered: true }));
    store.set("abstractobserver_entity_token", "legacy-tok");
    expect(loadStoredAuth()).toBeNull();
    expect(store.has(AUTH_STORAGE_KEY)).toBe(false);
    expect(store.has("abstractobserver_gateway_auth")).toBe(false);
    expect(store.has("abstractobserver_entity_token")).toBe(false);
  });

  it("session-mode states are never persisted (the proxy cookie is the truth)", () => {
    const store = stubStorage();
