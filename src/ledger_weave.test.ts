...
describe("pair-trail ledger lines", () => {
  ...
  it('diary_read host markers carry the "repeat" group (fold the book-read wall)', () => {
    const line = ledgerLine({
      stream: "abstractmemory.replay",
      stream_version: 1,
      seq: 4778.0125,
      ...
      turn_id: null,
      run_id: null,
      payload: { kind: "diary_read", channel: "operator", reason: "cognition calibration" },
    } as unknown as ReplayEnvelope);
    expect(line.title).toBe("The book was read");
    expect(line.groupKind).toBe("repeat");
    expect(line.members).toEqual(["operator — cognition calibration"]);
  });
