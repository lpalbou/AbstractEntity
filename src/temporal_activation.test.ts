...
describe("temporal activation (engine parity)", () => {
  ...
  it("presentation intensities spread to the engine ceiling (burst-axis recalibration, deep-check 2026-07-27)", () => {
    // One fresh use = clearly visible (0.45); saturation only at the
    // engine's clamp ceiling — "used once lately" and "used constantly"
    // must render differently (the all-green finding: the old 2x-fresh
    // saturation collapsed activations 8-25 into one identical green).
    expect(nodeWarmth(0)).toBe(0);
    expect(nodeWarmth(8)).toBeCloseTo(0.45, 5);
    expect(nodeWarmth(16)).toBeCloseTo(0.45 + 0.55 * (8 / 17), 5);
    expect(nodeWarmth(25)).toBe(1);
    expect(pairWarmth(4)).toBeCloseTo(0.45, 5);
    expect(pairWarmth(8)).toBeCloseTo(0.45 + 0.55 * (4 / 21), 5);
    expect(pairWarmth(25)).toBe(1);
    // Monotone: recency ordering is never inverted by the map.
    expect(pairWarmth(12)).toBeGreaterThan(pairWarmth(8));
    expect(nodeWarmth(20)).toBeGreaterThan(nodeWarmth(16));
  });
...
describe("head_burst (the wash measure — fable5 verdict on laurent c5439)", () => {
  it("counts the newest commit's deposit across streams and splits selected", () => {
    const buffers = new Map<string, AttentionEventLite[]>();
    const ev = (kind: string, seq: number, trace: string | null): AttentionEventLite => ({
      kind,
      record_id: kind === "selected" ? `r${seq}` : null,
      pair_key: kind === "co_selected" ? `a|b${seq}` : null,
      weight: 1,
      ttl_activity: null,
      seq,
      trace_id: trace,
    });
    buffers.set("life|e", [ev("selected", 1, "t1"), ev("selected", 10, "t2"), ev("co_selected", 11, "t2"), ev("co_selected", 12, "t2")]);
    const t = computeTemporalActivation(buffers);
    expect(t.head_burst).toEqual({ total: 3, selected: 1, trace_id: "t2" });
  });

  it("no trace on the newest event: burst is null (never guessed)", () => {
    const buffers = new Map<string, AttentionEventLite[]>();
    buffers.set("life|e", [{ kind: "selected", record_id: "r", pair_key: null, weight: 1, ttl_activity: null, seq: 5, trace_id: null }]);
    expect(computeTemporalActivation(buffers).head_burst).toBeNull();
  });
});
