...
export interface NodeState {
  ...
  /** Digest provenance when served (mechanical-v1/-v2, mechanical-flow-v1, …). */
  ...
  /** The diary entry's TYPE (note/reflection/question/problem/…) when the
   * stream's display block carries it — the Questions lens + card keys.
   * null until memory ships diary_type on diary displays (dm#27). */
  ...
  /** Dream signals (wave-5, memory ship c3724): the night's telemetry —
   * what the sleep pass MOVED, carried VERBATIM on the dream's display
   * block (bounded ≤12; fragments ≤200ch from touched records' TITLES —
   * private diary words structurally never enter). Absent = pre-signal
   * dream, self-identifying, zero migration. */
  signals: DreamSignal[] | null;
}

/** One night-signal (frozen shape, commons 3708): structure decides,
 * feelings color — `felt` is OPTIONAL (absent = nothing was ever felt
 * toward the touched targets, or a machine act, which never carries it). */
export interface DreamSignal {
  kind: string; // changed_understanding | unresolved_tension | changed_navigation
  phase?: string; // resolution | world_models | maintenance | mining | dream
  act?: string; // dream_resolved | card_formed | card_revised | grouped | candidate_minted | bridge_proposed
  fragment?: string;
  touched?: string[];
  felt?: { tone?: string; weight?: number; scarred?: boolean; bonded?: boolean };
}
...
function applyDisplayToNode(node: NodeState, display: DisplayBlock): void {
  ...
  if (display.kind === "diary" || display.diary === true) {
    ...
    // The entry's TYPE (question/problem/…) when the stream carries it —
    // the Questions lens keys on it (dm#27 forensics: elected diary
    // questions fold as kind="diary" and were invisible to every lens;
    // memory is adding diary_type to the display block — type only, the
    // words stay in the book). Absent until that ships; never guessed.
    const dt = (display as { diary_type?: unknown }).diary_type;
    if (typeof dt === "string" && dt && !node.diary_type) node.diary_type = dt;
    return;
  }
  ...
  // Dream signals ride the display VERBATIM (memory c3724, lane (a) —
  // block.signals == the record's stream, pinned at the pen). Tolerant:
  // shape-checked minimally, unknown kinds kept (verbatim honesty), only
  // arrays of objects accepted.
  const sig = (display as { signals?: unknown }).signals;
  if (Array.isArray(sig) && sig.length > 0 && !node.signals) {
    const rows = sig.filter((s): s is Record<string, unknown> => typeof s === "object" && s !== null);
    if (rows.length > 0) node.signals = rows as unknown as DreamSignal[];
  }
  // Digest provenance (flow-brain tasking c5190/c5202): a record's
  // digest_method names WHO wrote its words (mechanical-v1/v2 = the
  // driver's floor, mechanical-flow-v1 = the flow brain, entity prose =
  // absent). Render-when-present; the consent set (c2447 pact) is
  // memory's, never re-declared here.
  const dm = (display as { digest_method?: unknown }).digest_method;
  if (typeof dm === "string" && dm && !node.digest_method) node.digest_method = dm;
...
function resolveUsageNode(state: FoldState, rowId: string, display: DisplayBlock, seq: number): NodeState {
  ...
  if (!node) {
    ...
    // node's metadata in and drop the duplicate. FULL metadata set (code
    // adversary F11: this direction copied less than its twin at the
    // re-key branch — entry_id/born_at/token_estimate/visit_id/bookkeeping
    // were dropped, so the diary "Read the entry" door disappeared
    // whenever the row id was revealed before the graph id).
    ...
    if (graphNode && graphNode !== node) {
      ...
      if (!node.entry_id) node.entry_id = graphNode.entry_id;
      if (!node.born_at) node.born_at = graphNode.born_at;
      if (!node.token_estimate) node.token_estimate = graphNode.token_estimate;
      if (!node.visit_id) node.visit_id = graphNode.visit_id;
      node.bookkeeping = node.bookkeeping || graphNode.bookkeeping;
      state.nodes.delete(graphId);
      rekeyNodeInEdges(state, graphId, rowId);
    } else {
      node.graph_id = graphId;
...
/** The considered-candidates count of one trace payload — ONE reading rule
 * for every surface (ledger line, pulse bars, inspector beat; the
 * clamp-drift lesson applied to display). Prefers the engine's own
 * accounting (budget_spent.candidates_considered, which rides the trace
 * whole) over the length of the `candidates` LIST: the list is
 * display-bounded, and counting it was the artifact the operator's
 * 2026-08-01 within-turn audit caught — the UI said 64 while the true
 * pools ran 321-463 ("let's make it at most a 100" capped the pool; this
 * reader makes the count honest even when a universe outgrows the list,
 * e.g. via spreading). Falls back to the list length; null when the trace
 * shipped neither (absent-not-zero: callers choose their own absent
 * rendering — a fabricated denominator was the pulse adversary's N2). */
export function consideredCount(payload: unknown): number | null {
  const p = (payload ?? {}) as Record<string, unknown>;
  const spent = p["budget_spent"];
  if (spent && typeof spent === "object" && !Array.isArray(spent)) {
    const n = (spent as Record<string, unknown>)["candidates_considered"];
...
function applyTrace(state: FoldState, env: ReplayEnvelope): void {
  ...
  beat.candidate_count = consideredCount(p) ?? 0;
 ...
 *
 * The fold runs inside a render-path memo above the panel error
 * boundaries, so a throw here takes down the WHOLE app for that life
 * (code adversary F1/F2). Ingest normalization (normalizeEnvelope) is the
 * first line; this per-envelope try/catch is defense-in-depth for any
 * malformed payload SHAPE a family applier still dereferences (e.g. a null
 * item inside a payload array) — a single bad envelope is skipped, the
 * life stays openable. `seq` must be FINITE (Infinity poisons the
 * dup-guard and the live cursor, F3). */
export function applyEnvelope(state: FoldState, env: ReplayEnvelope): void {
  if (!Number.isFinite(env.seq)) return;
  ...
  try {
    ...
    // A malformed payload shape must never take down the fold — skip this
    // envelope, keep the life renderable. seq/applied_count still advance
    // below so the dup-guard and cache stay consistent.
    if (typeof console !== "undefined") console.warn(`#FALLBACK: skipped malformed ${env.family} envelope at seq ${env.seq}`, e);
  }
  state.seq = env.seq;
...
/** Cheap recovery for RETROGRADE HOST-MARKER INSERTS (code adversary F3):
 * the live merge binary-inserts late host markers (fractional seqs)
 * mid-array, which breaks the pure-extension check and used to force a
 * full O(N) refold — a UI freeze per marker at operator scale. But
 * `applyHost` only APPENDS to `state.sessions`, so when the ONLY
 * difference behind the covered head is K host-family envelopes, the
 * truthful fold is reproducible by inserting their session entries in seq
 * order — PROVABLY: we count host envelopes in the prefix missing from
 * sessions; if that count ≠ the index shift K, a non-host insert hid in
 * there and we rebuild. Inserted markers carrying a run_id that would
 * have changed the session-inference walk also fall back to the rebuild
 * (the fast path must be exactly what the full fold would produce, or it
 * does not run). */
function tryHostRetrogradeRecovery(envelopes: ReplayEnvelope[], cache: FoldCache, target: number): FoldCache | null {
  // Locate the previously-covered head in the grown array (seq-ordered).
  let lo = 0;
  let hi = Math.min(target, envelopes.length - 1);
  let j = -1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    ...
    if (s < cache.last_seq) lo = mid + 1;
    else hi = mid - 1;
  }
  if (j < 0) return null;
  const k = j - cache.index;
  if (k <= 0 || k > 16) return null; // 0 = not an insert; >16 = rebuild is fine
  const known = new Set<number>();
  for (const s of cache.state.sessions) known.add(s.seq);
  const inserted: Array<(typeof cache.state.sessions)[number]> = [];
  let lastRun: string | null = null;
  for (let i = 0; i <= j; i++) {
    const env = envelopes[i];
    if (env.family === "host" && !known.has(env.seq)) {
      // An inserted marker whose run_id would have shifted the inference
      // walk changes more than sessions — the fast path refuses.
      if (env.run_id && env.run_id !== lastRun) return null;
      const p = env.payload as unknown as HostPayload;
      const { kind, session_id, ...details } = p;
      inserted.push({
        kind: String(kind || ""),
    ...
    if (env.run_id) lastRun = env.run_id;
  }
  if (inserted.length !== k) return null; // a non-host insert is hiding
  for (const entry of inserted) {
    // Sorted insert (sessions are seq-ordered; K is tiny).
    let at = cache.state.sessions.length;
    while (at > 0 && cache.state.sessions[at - 1].seq > entry.seq) at -= 1;
    cache.state.sessions.splice(at, 0, entry);
    cache.state.applied_count += 1;
  }
  for (let i = j + 1; i <= target; i++) applyEnvelope(cache.state, envelopes[i]);
...
export function foldUpToIndex(envelopes: ReplayEnvelope[], uptoIndex: number, cache: FoldCache | null): FoldCache {
  ...
  if (cache && cache.index >= 0 && target >= cache.index) {
    const recovered = tryHostRetrogradeRecovery(envelopes, cache, target);
