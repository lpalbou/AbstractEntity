...
describe("stream fold", () => {
  ...
  it("candidate counts prefer the engine's budget_spent.candidates_considered over the capped list", () => {
    // Operator 2026-08-01 within-turn audit: every surface counted the
    // trace's display-bounded candidates list (cap 64) while the true
    // pools ran 321-463. One reading rule now (consideredCount): the
    // engine's own accounting wins wherever it rides the trace; the list
    // length stays the fallback for older persisted traces.
    const tracePayload = (traceId: string, over: Record<string, unknown>) => ({
      trace_id: traceId,
      trace_kind: "reconstruct",
      need: { cue_text: "wide pool", turn_id: "turn-9", view: "working_set" },
      searched_scopes: [],
      channels: ["keyword"],
      candidates: [{ record_id: "a", scores: {} }, { record_id: "b", scores: {} }],
      selected: ["a"],
    ...
    const trueEnv = env({
      ...
      seq: 1,
      trace_id: "t-true",
      payload: tracePayload("t-true", { budget_spent: { candidates_considered: 321, token_budget: 6000 } }),
    });
    const legacyEnv = env({
      family: "trace",
      seq: 2,
      trace_id: "t-legacy",
      payload: tracePayload("t-legacy", { seq: 2 }),
    });
    const fold = foldEnvelopes([trueEnv, legacyEnv]);
    // The inspector's beat reads the truth...
    expect(fold.beats.get("t-true")!.candidate_count).toBe(321);
    // ...an older trace without the accounting keeps the list-length read.
    expect(fold.beats.get("t-legacy")!.candidate_count).toBe(2);
    // The ledger line says the same number as the beat (one truth, every
    // surface — the artifact was surfaces disagreeing with the engine).
    expect(ledgerLine(trueEnv).detail).toContain("321 considered");
    expect(ledgerLine(legacyEnv).detail).toContain("2 considered");
  });
...
describe("incremental fold cache", () => {
  ...
  it("retrograde HOST insert recovers WITHOUT a rebuild and equals the full fold (F3)", () => {
    const envelopes = life();
    let cache = foldUpToIndex(envelopes, envelopes.length - 1, null);
    const stateBefore = cache.state;
    // A late host marker lands mid-array (fractional seq, run_id-less —
    // the gateway host-stream shape the merge inserts).
    const marker = env({ family: "host", seq: 3.001, payload: { kind: "wake", session_id: "s2" } });
    const grown = [...envelopes.slice(0, 4), marker, ...envelopes.slice(4)];
    cache = foldUpToIndex(grown, grown.length - 1, cache);
    expect(cache.state).toBe(stateBefore); // recovered in place — NOT rebuilt
    expect(cache.state.sessions.map((s) => s.seq)).toEqual([1.001, 3.001]); // sorted insert
    expect(snapshot(cache.state)).toBe(snapshot(foldEnvelopes(grown)));
  });

  it("retrograde NON-host insert falls back to the truthful rebuild", () => {
    const envelopes = life();
    let cache = foldUpToIndex(envelopes, envelopes.length - 1, null);
    const stateBefore = cache.state;
    const intruder = selectedEnv("row-a", 3.5, { record_id: "row-a", kind: "memory", title: "a", graph_id: "ex:memory-a" });
    const grown = [...envelopes.slice(0, 4), intruder, ...envelopes.slice(4)];
    cache = foldUpToIndex(grown, grown.length - 1, cache);
    expect(cache.state).not.toBe(stateBefore); // rebuilt — the fast path refused
    expect(snapshot(cache.state)).toBe(snapshot(foldEnvelopes(grown)));
  });

  it("retrograde host insert with a NEW run_id falls back (session inference would shift)", () => {
    const envelopes = life();
    let cache = foldUpToIndex(envelopes, envelopes.length - 1, null);
    const stateBefore = cache.state;
    const marker = env({ family: "host", seq: 3.001, run_id: "run-ghost", payload: { kind: "wake", session_id: "s2" } });
    const grown = [...envelopes.slice(0, 4), marker, ...envelopes.slice(4)];
    cache = foldUpToIndex(grown, grown.length - 1, cache);
    expect(cache.state).not.toBe(stateBefore); // rebuilt for inference truth
    expect(snapshot(cache.state)).toBe(snapshot(foldEnvelopes(grown)));
  });
});
...
describe("bookkeeping markers (plan item 3, observer half)", () => {
  ...
  it("folds dream signals VERBATIM from the display block (wave-5, memory c3724 lane (a))", () => {
    const signals = [
      { kind: "unresolved_tension", phase: "dream", act: "bridge_proposed", fragment: "what persists when no one is reading", touched: ["ex:diary-1"], felt: { tone: "warm", weight: 2.0 } },
      { kind: "changed_understanding", phase: "world_models", act: "card_formed", fragment: "admin card taking shape", touched: ["ex:world_model-1"] },
    ];
    const fold = foldEnvelopes([
      bindingEnv("ex:dream-sig", 1, {}, { record_id: "ex:dream-sig", kind: "dream", title: "dream: the night moved", signals }),
      bindingEnv("ex:dream-old", 2, {}, { record_id: "ex:dream-old", kind: "dream", title: "dream: pre-signal era" }),
    ]);
    const withSig = fold.nodes.get("ex:dream-sig")!;
    expect(withSig.signals).toHaveLength(2);
    expect(withSig.signals?.[0].kind).toBe("unresolved_tension");
    expect(withSig.signals?.[0].felt?.tone).toBe("warm");
    // The second signal is a machine-adjacent act with NO felt block —
    // the render's no-color rule keys on exactly this absence.
    expect(withSig.signals?.[1].felt).toBeUndefined();
    // ABSENT stays absent: pre-signal dreams self-identify (zero migration).
    expect(fold.nodes.get("ex:dream-old")!.signals).toBeNull();
  });
